...
from autofocus_function.models import FixedRangeAutofocusParams, ScoredZPoint
from autofocus_function.protocols import (
    FrameProvider,
...
class FixedRangeScanner:
    ...
    def scan(
        ...
        return self.scan_step(roi, on_progress=on_progress)
    ...
    def sample(
        ...
        *,
        ...
        retention_provider = (
            self.frames if isinstance(self.frames, FrameRetentionProvider) else None
        )
        if retention_provider is not None and captured_frames:
            keep_index = min(
                range(len(captured_frames)),
                key=lambda idx: (abs(scores[idx] - median_score), idx),
            )
            retention_provider.retain_only(captured_frames, captured_frames[keep_index])
    ...
    def build_verification_scanner(self, peak_z_um: float) -> "FixedRangeScanner":
        verification_range_um = float(self.params.verification_range_um)
        verification_params = replace(
            self.params,
            scan_mode="step",
            z_start_um=peak_z_um + verification_range_um,
            z_end_um=peak_z_um - verification_range_um,
            point_count=int(self.params.verification_point_count),
            frames_per_z=int(self.params.verification_frames_per_z),
        )
        return FixedRangeScanner(self.stage, self.frames, self.strategy, verification_params)
